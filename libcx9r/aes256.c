...
void cx9r_aes256_ecb_init(cx9r_aes256_ecb_ctx *ctx, uint8_t *key)
{
  ...
  gcry_cipher_setkey(*ctx, key, CX9R_AES256_KEY_LENGTH);
}

void cx9r_aes256_ecb_encrypt(cx9r_aes256_ecb_ctx *ctx, uint8_t *block)
{
  gcry_cipher_encrypt(*ctx, block, CX9R_AES256_BLOCK_LENGTH, NULL, 0);
}
...
void cx9r_aes256_cbc_init(cx9r_aes256_cbc_ctx *ctx, uint8_t *key, uint8_t *iv)
{
  gcry_cipher_open(ctx, GCRY_CIPHER_AES256, GCRY_CIPHER_MODE_CBC, 0);
  gcry_cipher_setkey(*ctx, key, CX9R_AES256_KEY_LENGTH);
  gcry_cipher_setiv(*ctx, iv, CX9R_AES256_BLOCK_LENGTH);
}
...
void cx9r_aes256_cbc_close(cx9r_aes256_cbc_ctx *ctx)
{
  gcry_cipher_close(*ctx);
}
